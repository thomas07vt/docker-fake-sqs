...
if [ -n "$VIRTUAL_HOST" ]
then
  exec fake_sqs --bind 0.0.0.0 --hostname $VIRTUAL_HOST --database=/messages/sqs/database.yml --port 9494 --server thin -v
else
  exec fake_sqs --bind 0.0.0.0 --database=/messages/sqs/database.yml --port 9494 --server thin -v
fi
